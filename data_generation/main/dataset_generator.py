...
def get_dataset(corpus, fns, n_cands=2):
    ...
    for fn, thread in zip(fns, corpus):
        ...
        for index, (line, adr_id) in enumerate(zip(thread, adr_ids)):  # sent: [time, name, tokens]
            pos_response = line[2]
            adr_label = '-'
            responses = [pos_response] + ['-' for i in xrange(n_cands-1)]
            label = '-'
...
def get_neg_responses(thread, indices, pos_sample_index, n_cands=1):
    ...
    return [thread[neg_indices[i]][2] for i in xrange(n_cands)]
...
def get_addressee_info(thread, speaker_ids):
    ...
    for i, line in enumerate(thread):
        ...
        if n_words > 2 and len(utterance[1]) == 1 and PUNCT.match(utterance[1]) and adr_id in speaker_ids:
...
def tuning(data):
    return [[sent for sent in sents if len(sent[2]) > 0 and sent[1] != "*"] for sents in data]
...
def main(argv):
    ...
    assert len(dataset) == len(fns), 'dataset: %d  fns: %d' % (len(dataset), len(fns))
